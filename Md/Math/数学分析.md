...
一、计算题每小题10分, 共40分)
1. 求 
  $$ \lim_{x \to 0} \frac {\int _ {0}^ {\sin x}(1+t)^ {\frac {1}{t}}dt}{ \int _ {x}^{0}{\frac{\sin}{t}} }dt $$
2. 求 
  $$ \lim _ {x\to 0}  \frac { \tan ( \tan x)- \sin ( \sin x} {x( e^ {-x^ {2}}-1)} $$ 
3. 设 $y= \tan x^ {2}  \sqrt {x+\sqrt {x+3}} $ 求$ y''$
4. 求 $ \int _ {0}^ {1}  \frac {dx}{(1+e^ {x})^ {2}} $ 
二、已知$ y=e^ {ty} +x $ 而t是由方程$y^2-t^2-x^2=1 $ 确定的x、y的函数，求$\frac{dy}{dx}$
三、f(u)连续,
t)= \int _ {0}^ {\frac {1}{2}} f(
 \lim _ {t\to 0^ {+}}  \frac {F(t)}{t^ {3}} 
四、设C是|x|+|y| \leqslant 1边界曲
线,取正向. 
...
八、 证明:若f(x)在[a, \infty ),上连续,且 \lim _ {x\to +0} f (x ) = b,则函数在f (x ) 在[a, \infty )
上一致连续。
九、设p(x)在[a,b]上非负且连续,f(x)、g(x)在[a,b]上连续且单调增加,证
明: \int _ {a}^ {b} p(x)f(x)dx \int _ {a}^ {b} p(x)f(x)g(x)dx \leqslant  \int _ {a}^ {b} p(x)dx \int _ {a}^ {b} p(x)g(x)dx

## 200# 2002
一、计算题(每题5分,共
1. 已知 \lim _ {x\to 0}  \frac {\sqrt {1+f(x)\sin x}-1}{e^ {2x}-1} =1,求imf(x)
...
## 200# 2003
1. 求数列极限 \lim _ {n\to \infty } ( \frac {1}{n^ {2}+n+1} 
 \frac {3}{n^ {2}+n+3} + \cdots + \frac {n}{n^ {2}+n+n} )
2. 求数列极限 \lim _ {n\to 00} s i n ( \pi  \sqrt {n^ {2}} 
...
立方体表面取外侧。
 二、 给出下列极限的定义
1. \lim _ {x\to +\infty } f (x ) = \infty 
2. 当x \to a+0时,f(x)不以A为极限
...
六、(14分)设函数f(x)在[a,b]
证明存在一点
使得|f'( \xi )| \geqslant  \frac {4}{(b-a)^ {2}} |f(b) \overto {A} (a|)

## 200# 2004
1. 求数列极限 \lim _ {n\to 10} ( \sin n) ( n^ {2} +1)
| \frac {2n^ {2}+1}{n^ {2}-1} 
2. 求函数极限 \lim _ {x\to \infty } (s i n \sqrt {x+1} -s i n x \sqrt {x} 
...
三、 证明:若函数f(x)在[a,+ \infty )连续,且 \lim _ {x\to 00} [ b x-f (x ) ] =0, 其中b 是非零常
数,则函数f(x)
四、 证明:若 f_ {+} (a)与f(a)都存在,则函数f(x)在a连续
五、(15)设f(x)在[0,1]上连续,f(0)=f(1)。证明对任何正整数n存在一点 \xi  \in [0,1] ,
使得f( \xi + \frac {1}{n} )=f( \xi )。
设 u_ {n} (x)= \frac {1}{n^ {3}}  \ln (1+ n^ {2}  x^ {2} ),n=1,2,
1. 证明函数项级数 \sum _ {i=1}^ {n}  u_ {n} (x)在[0,1]一致收敛;
2. 讨论其和函数在[0,1]上的连续性、可积性和可微性。

## 2005
1. \lim _ {n\to 0}  \frac {1}{3}  \frac {+\frac {1}{2}^ {2}+\cdots +\frac {1}{2}}{\frac {1}{3}+\frac {1}{3}+\cdots \frac {1}{3^ {n}}} 
2. \lim _ {x\to 0}  \frac {\sqrt {1+x}-\sqrt {1-x}}{\sqrt [3]{1+x}-\sqrt [3]{1-x}} 
 二、计算满足小列条件
三、每个 \_ 明
f(x)= \cos 
s \sqrt {x} . 在[
...
五、 讨论下列无穷积分的收敛
、 求下列幂级数的收致半径下
七、设f(u,v)所有二阶偏导数都连续,z=f(xy, \frac {y}{x} ),求 \frac {a^ {2}z}{axay} 
八、求抛物线 y^ {2} =mx, y^ {2} =nx和直线y= \alpha x,y= \beta x所围成的区域D的面积
S(D),(0<m<n,0< \alpha < \beta )
九、计算 \int _ {\frac {y}{y}}^ {\frac {1}{1}} f( \frac {x}{y} )dydz+ \frac {1}{x} f( \frac {x}{y} )dzdx+zdxdy,其中f(u)连续可微,S为柱面
 \_ 
 \_ 
+ y^ {2} 
...
5 4. 
分)
1. 求极限 \lim _ {x\to 0}  \frac {a}{x} -( \frac {1}{x^ {2}} - a^ {2} ) \ln (1+ax),a \neq 0
2. 
...
## 2007
1. 求数列极限 \lim _ {n\to 0}  \frac {1}{1\cdot 2\cdot 3} + \frac {\times 1}{2\cdot 3} 
2. 求函数极限 \lim _ {x\to \infty }  \sin x \frac {2}{x} |
...
, 2#p平
了z=1所割下的部分
10. 设f(x)在点x=0的某邻域具有二阶连续导数,且 \lim _ {x\to 0}  \frac {f(x)}{x} =0,试判别级数 \sum _ {n=1}^ {co} f( \frac {1}{n} )
的收敛性。
...
=0所确定的函数,求 \frac {Ou}{ax} |x=0,y=
 二、已知曲线C: \begin{cases}x^{2}+y^{2}-2z^{2}=0\\x+y+3z=5\end{cases} ,求曲线C距离xoy平面最远的点和最近的
点。
三、设函数f(x)在区间[-1,1]上有连续的一阶导数,且 \lim _ {x\to 0}  \frac {f(x)}{x} =a>0,证明
...
## 2009
1. 设函数
计算极限 \lim _ {x\to 0} [ 1+l n (1+x ) \frac {2}{x} 
问a为何值时,f(x)在x=
3. 设函数f(x)在闭区间[a,b]上连续,它
生开区
且f'(x)>0。若
 \lim _ {x\to a}  \frac {f(2x-a)}{x-a} 存在,证明:
(1)在(a,b)内,f(x)>0;
...
## 2010
1. 求 \lim _ {x\to 0}  (\frac {\sin x}{x} )^ {\frac {1}{1-\cos x}} ,
...
## 2011
一、计算题(1-9小题每题各10分,共90分)
1. 求极限 \lim _ {n\to 0(1)\frac {1}{2}} + \frac {3}{2^ {2}} + \cdots  \frac {2n-1}{3}  2^ {n} )
2. 求极限 \lim _ {x\to 0}  (x+A+x^ {2} )^ {\ln } 
...
10. 设f(x)在[a, \infty ]上连续,且 \lim _ {x\to 0} f(x)=A(其中A为常数),证明:f(x)在[a, \infty ]]
? 大/于
...
## 2012
1. 当 \alpha =?时, x^ {\alpha } 与 \frac {1}{1+x} '(1-x)是x \to 0时的同阶无穷水。求 \alpha 。
2. 求 \lim _ {x\to 1}  \frac {\sin ^ {2}(1-x)}{(x-1)^ {2}(x+3)} 。
...
三、设f(x)在x=0连续,且 \lim _ {x}  \frac {f(x)}{x} =7,求试
正f(x)在x=0处可导,并求f(0)
四、设f为可微函数,u=f( x^ {2} + y^ {2} + z^ {2} ),方程3x+ 2y^ {2} + z^ {3} =6xyz确定了隐函
数z=z(x,y),求 \frac {au}{ax} |(1,1,1)。
五、求锥面z= \sqrt {x^ {2}+y^ {2}} 与球面 x^ {2} + y^ {2} + (z-a)^ {2} = a^ {2} 所围立体的体积。
一、应用格林公式计算曲线积分 \frac {xdy-ydx}{x^ {2}+y^ {2}} ,其中L为任意包含原点的闭区
域的边界曲线。
七、 设 a^ {2} -3b<0,
有一立根
八、设 \sum _ {n=1}^ {o}  a_ {n} 收致,且 \lim _ {n\to 0} n a_ {n} =0,证明: \sum _ {n=1}^ {o} n( a_ {n} - a_ {n+1} )= \sum _ {n=1}^ {c}  a_ {1} 
、(17分)设f(x)在[0,1]上连续,f(1)=0,证明:
(1) x^ {n} ,在[0,1]上不一致连续;
(2)f(x) x^ {n} 在[0,1]上一致连续。

## 2013
1. 求极限 \lim _ {x\to \frac {2\sin ^ {2}}{2\sin ^ {2}}x+\sin x-1}{3}  \sin x+
2. 设f(x)={k,
EABCx
处简
导,并求
...
关的标记】
1. 
计算数列极限 \lim _ {n\to 0}  \frac {1}{2} + \frac {3}{2^ {2}} 
2. 计算函数极限 \lim _ {x\to -1} 
( \frac {\sqrt {2}}{2} )
3. 导数计算(x)
\begin{matrix}2x&
4. 判断下列瑕积分是否收敛, 若收敢,
...
13. 证明:若L为平面上封闭曲线, \overline {l} 养任意方向,则 \widehat {l}  \cos ( \overto {l} , \overto {n} )ds=0,其中 \overto {n} 为曲线L
的外法线方向
说明:1-9小题每题各10
...
、保持卷面清洁。一律使用黑
色或蓝色字迹的钢笔或签字笔(同一科目答卷的字迹必须是一种颜色);答题纸上禁山
计算数列极限 \lim _ {n\to 0} (1 + \frac {1}{n} - \frac {1}{n} 
2. 计算函数极限 \lim _ {x\to -1} 
...
计算 \lim _ {i\to \infty }  x_ {1} 
2. 计算 \lim _ {x\to -1}^ {2n^ {2}+4\times 2n^ {2}+2} 
3. 设f(x)可导,求函
...
9. 计算I= \beta  \frac {e^ {x}}{\sqrt {x^ {2}+y^ {2}}} dxdy,其中 \sum 为锥面z= \sqrt {x^ {2}+y^ {2}} 与平面z=1,z=2
所围立体的边界曲面, 取外侧。
10. 试叙述函数f(x)在区间I上连续与一致连续的定义,说明二者的区别
11. 判断函数项级数 \sum _ {n-1}^ {\sin x\sin nx} ,x \in (- \infty , \infty )的一致收致性
12. 证明函数f(x,y)=
 x^ {2} + y^ {2}  \neq 0在点(0,0)连续且偏
导数存在,但偏导数在(0,0)不连续,而. 
13. 设f(x)在区间[0,1]上连续,在(0,1)内可导,且 \int _ {0}^ {1} f(x)|x=0,证明:存在
...
( \frac {1+x}{1-e} )
3. 
 设函数f(x)= \lim _ {1\to n^ {m}}  \frac {x^ {2}e^ {n(x-1)}+}{e^ {n(x+1)}} 
4. 
...
10. 设f(x) \sim g(x)(x \to  x_ {0} ),证明:
f(x)-g(x)=o(g(x))
11.  设f(x)在区间[a,b]上连续,在(a,b)内可导,f(a)=f(b)=0
试证: \forall  \alpha  \in R, \exists  \xi  \in (a,b)使得 \alpha f( \xi )=f( \xi )
12. 证明:若L为平面上封闭光滑曲线, \overline {l} 为平面上任意方向向量,则沿着L
正方向的线积分, \cos (1, \overto {n} )ds=0,其中 \overto {n} 为曲线的单位外法向量
13. 设函数P(x,y,z),Q(x,y,z),R(x,y,z)在 R^ {3} 上具有连续偏导数,且对任意光滑
曲面 \sum ,成立 \int _ {2}^ {0} Pdydz+Qdzdx+Rdxdy=0,证明:在 R^ {3} 上, \frac {aP}{ax} + \frac {aQ}{ay} + \frac {3R}{az} =0。

## 2018
1. 用 \varepsilon -N方法证明 \lim _ {n\to 4}^ {12} 
 \frac {n^ {2}}{n^ {2}+n+1} =1. 
2. 叙述函数f(x)在区
AE有
9
...
(- \infty ,+ \infty )的有界
3. 求函数f(x)= |2x^ {3} 
4. 叙述极限 \lim _ {x\to +0} f (x ) =a
5. (15)设f(x)在[0,1]上连续,在(0,1)内可是,且3
 \int _ {0}^ {\frac {1}{3}}  \sqrt {1} (x)dx=f( \frac {1}{2} )。求证:
存在 \xi  \in (0,1),使得f'( \xi )=0。
6. 证明不等式In(x+y) \geqslant  \ln  \sqrt {xy} + \ln 2
(x,y>0). 
7. 计算 \sum _ {i=1}^ {\circ }  \frac {1}{n}  x^ {n} 的和函数并求 \sum _ {m=1}^ {o}  \frac {(-1)”}{n} . 
8. 验证函数f(x,y)= \begin{cases}\frac{2}{x^{2}}\\\end{cases} 
 \frac {2xy}{+y^ {2}} 
, x^ {2} + y^ {2}  \neq 0,在原点(0,0)分别
对每个自变数x或y都连续,但是二元函数在原点(0,0)却不连续
9. (15)证明对任意 \alpha >0, \int _ {0}^ {+} ( \alpha + x^ {2} ) \sin tdx关于t \in (0,+ \infty )均一致收致
0. 求球面 x^ {2} + y^ {2} + (z-a)^ {2} = a^ {2} 与锥面 x^ {2} ( xy^ {2} = z^ {2} 围成的体积
...
## 2019
1. 已知数列{ x_ {n} }的极限 \lim _ {n\to \infty }  x_ {1} =a,用 \varepsilon -N语言证明 \lim _ {n\to 00}  \sqrt [3]{x_ {n}} = \sqrt [3]{a} 
2. 求
求函数极限 lir_ {i} 
12000
4. 设f(x)不
证明 \lim _ {o\to x}  \frac {f(x)}{x} =0
5. 设函数f(x)
有二阶导数,
l对于任意的x \in (- \infty ,+ \infty )有
|f(x)| \leqslant a,|f'(x)| \leqslant b,则第|f'(x) |k^ {2}  \sqrt {ab} 
A. \sqrt {3} 
6. 求心脏线 \rho =a(1+ \infty )所围图开
的面
7. 求级数 \sum _ {n=1}^ {\circ }  \frac {1}{n(2n+1)} 的和
8. 考察函数f(x,y)= \begin{cases}\frac{x^{2}y^{2}}{(x^{2}+y^{2})^{2}},\\0,\end{cases}  x^ {2} + y^ {2}  \neq 0,在原点的连续性与可微性. 
9. 判别级数 \sum _ {i=1}^ {n}  \frac {(-1)^ {m+1}}{n^ {x}} 在区间[1,+ \infty )上是否一致收数
10. 计算定积分 \int _ {0}^ {a} arc \tan  \sqrt {\frac {\alpha -x}{\omega +x}} dx(a>0). 
11. 设f(x)在[0,+ \infty )上有连续的导数,并且f(0)=1. 令函数
H(a)= \int _ {x^ {2}+y^ {2}+2}  \leqslant  a^ {2} ( x^ {2} + y^ {2} + z^ {2} )dxdydz(a>0),
证明:函数H(a)在a=0
并求
 \times 
12. 设算曲面积分 \int _ {\frac {1}{s}}^ {\frac {xdydz+3}{2}}  x^ {2} + y^ {2} + z^ {2} ,其中
保由的面
 x^ {2} + y^ {2} = R^ {2} 以及两个平
公
面z=R, \widehat {z} =-R
